...
const status=$('status'), pairBox=$('pairBox'), codeInput=$('code'), mobileUrl=$('mobileUrl');
...
function showPairing(base,code){const clean=base.replace(/\/$/,'');codeInput.value=code;mobileUrl.textContent=`${clean}/mobile/?code=${encodeURIComponent(code)}`;pairBox.classList.add('show')}
async function createPairing(){
  ...
  try{
    ...
    showPairing(base,d.code);setStatus('✓ New secure pairing created. Scan the link or enter the code.',true);
...
clearPair.addEventListener('click',async()=>{await chrome.storage.local.remove(['pairCode','privateKey','publicKey']);pairBox.classList.remove('show');codeInput.value='';mobileUrl.textContent='';setStatus('Pairing cleared. Create a new secure pairing.',true)});
