...
const pairButton=$('pair'), newPairButton=$('newPair'), copyButton=$('copy'), clearPair=$('clearPair');
...
function showPairing(code){
  ...
  const link=`${BASE_URL}/mobile/?code=${encodeURIComponent(code)}`;
  mobileUrl.textContent=link;
  qr.src=`${BASE_URL}/api/pair/qr?code=${encodeURIComponent(code)}&t=${Date.now()}`;
...
async function createPairing(){
  ...
  try{
    ...
    const r=await fetch(`${BASE_URL}/api/pair/create`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({publicKey:keys.publicKey}),cache:'no-store'});
    const d=await r.json();
    if(!r.ok||!d.ok||!d.e2ee)throw new Error(d.error||'Could not create pairing');
    await setConfig({baseUrl:BASE_URL,pairCode:d.code,enabled:true,privateKey:keys.privateKey,publicKey:keys.publicKey});
    showPairing(d.code);setStatus('✓ New secure pairing created. Scan the QR code or enter the code.',true);
  }catch(e){console.error(e);setStatus(e.message||'Could not create secure pairing.')}
}
(async()=>{
  const c=await getConfig();
  if(c.baseUrl!==BASE_URL)await setConfig({baseUrl:BASE_URL});
  if(c.pairCode&&c.privateKey){
    try{
      const r=await fetch(`${BASE_URL}/api/pair/status?code=${encodeURIComponent(c.pairCode)}`,{cache:'no-store'});const d=await r.json();
      if(r.ok&&d.ok&&d.e2ee)showPairing(c.pairCode),setStatus('✓ E2EE pairing is active.',true);
