...
MOBILE=ROOT/'mobile'; DATA=ROOT/'data'; DATA.mkdir(exist_ok=True)
MAX_FILE=25*1024*1024; PAIR_TTL=24*60*60; FILE_TTL=10*60; CLEAN_INTERVAL=60
...
def remove_pair(code):
    with lock:
        item=pairs.pop(code,None)
        if item:
            for q in list(item.get('subscribers',())):
...
def cleanup_loop():
    while True:
        time.sleep(CLEAN_INTERVAL); cutoff_file=now()-FILE_TTL; cutoff_pair=now()-PAIR_TTL
        with lock: expired=[c for c,v in pairs.items() if v['created']<cutoff_pair]
        for c in expired: remove_pair(c)
...
class Handler(BaseHTTPRequestHandler):
    server_version='AISnapCloud/0.4.4'
    def log_message(self,fmt,*args): pass
    def cors(self):
        self.send_header('Access-Control-Allow-Origin','*'); self.send_header('Access-Control-Allow-Headers','Content-Type, X-AI-Snap-IV, X-AI-Snap-AES-Key, X-AI-Snap-Name'); self.send_header('Access-Control-Expose-Headers','X-AI-Snap-IV, X-AI-Snap-Key, X-AI-Snap-Mime, X-AI-Snap-Name'); self.send_header('Access-Control-Allow-Methods','GET, POST, OPTIONS'); self.send_header('Cache-Control','no-store'); self.send_header('X-Content-Type-Options','nosniff'); self.send_header('Referrer-Policy','no-referrer')
    ...
    def meta(self,code,name):
        with lock:
            item=pairs.get(code); return item.get('files',{}).get(name,{}) if item else {}
    def do_GET(self):
        path=urllib.parse.urlparse(self.path).path
        if path=='/health': return self.send_json({'ok':True,'name':'AI Snap Cloud','version':'0.4.4','e2ee':True,'instant':True})
        if path=='/': return self.send_json({'ok':True,'name':'AI Snap Cloud','version':'0.4.4','e2ee':True,'mobile':f'{public_base(self)}/mobile/'})
        if path=='/api/pair/status':
            code=self.code()
            if not valid_code(code):return self.send_json({'ok':False,'error':'invalid_or_expired_pair'},401)
            item=pairs[code]; return self.send_json({'ok':True,'code':code,'connected':True,'e2ee':True,'publicKey':item['public_key'],'expiresIn':max(0,int(PAIR_TTL-(now()-item['created'])))})
        if path=='/api/pair/qr':
            code=self.code()
            if not valid_code(code):return self.send_json({'ok':False,'error':'invalid_or_expired_pair'},401)
            try:
        ...
        if path in ('/mobile/','/mobile'):return self.serve_file(MOBILE/'index.html')
        if path.startswith('/mobile/'):
            target=(MOBILE/path[len('/mobile/'):]).resolve()
        ...
        if path=='/api/file-meta':
            code=self.code()
            if not valid_code(code):return self.send_json({'error':'invalid_or_expired_pair'},401)
            name=Path(urllib.parse.unquote(self.q().get('name',[''])[0])).name; f=pair_dir(code)/name
            if not f.exists() or not f.is_file():return self.send_json({'error':'not_found'},404)
            m=self.meta(code,name)
            if not m:return self.send_json({'error':'metadata_not_found'},404)
            return self.send_json({'ok':True,'name':name,'iv':m.get('iv',''),'wrappedKey':m.get('wrapped_key',''),'mime':m.get('mime','image/jpeg'),'originalName':m.get('name',name)})
        if path.startswith('/api/file/'):
            code=self.code()
            if not valid_code(code):return self.send_json({'error':'invalid_or_expired_pair'},401)
            name=Path(urllib.parse.unquote(path[len('/api/file/'):])).name; f=pair_dir(code)/name
            if not f.exists() or not f.is_file():return self.send_json({'error':'not_found'},404)
            data=f.read_bytes(); ext=f.suffix.lower(); ct={'.jpg':'image/jpeg','.jpeg':'image/jpeg','.png':'image/png','.webp':'image/webp'}.get(ext,'application/octet-stream'); m=self.meta(code,name)
            return self.send_bytes(data,200,ct,{'Content-Disposition':f'inline; filename="{m.get("name",f.name)}"','X-AI-Snap-IV':m.get('iv',''),'X-AI-Snap-Key':m.get('wrapped_key',''),'X-AI-Snap-Mime':m.get('mime',ct),'X-AI-Snap-Name':m.get('name',f.name)})
    ...
    def do_POST(self):
        ...
        if path=='/api/pair/create':
            ...
            # A PC has exactly one active pairing. Creating a new one immediately
            # revokes every previous code, preventing stale phones from uploading
            # to an old pairing and causing 401/decryption mismatches.
            with lock: old=list(pairs.keys())
            for c in old:remove_pair(c)
            with lock:
    ...
    def upload(self):
        ...
        data=self.rfile.read(length); iv=self.headers.get('X-AI-Snap-IV'); key=self.headers.get('X-AI-Snap-AES-Key'); orig=self.headers.get('X-AI-Snap-Name','ai-snap.jpg')
        if not iv or not key:return self.send_json({'error':'missing_encryption_metadata'},400)
        orig=Path(urllib.parse.unquote(orig)).name or 'ai-snap.jpg'; ext='.jpg' if 'jpeg' in ct else '.png' if 'png' in ct else '.webp' if 'webp' in ct else '.bin'; name=f'snap_{int(now()*1000)}_{secrets.token_hex(3)}{ext}'; d=pair_dir(code); d.mkdir(exist_ok=True); (d/name).write_bytes(data)
        meta={'name':orig,'mime':ct,'iv':iv,'wrapped_key':key}
        with lock:pairs[code]['files'][name]=meta; pairs[code]['last_used']=now(); subs=list(pairs[code]['subscribers'])
        event={'name':name,'iv':iv,'wrappedKey':key,'mime':ct,'originalName':orig}
        for q in subs:
...
if __name__=='__main__':
    port=int(os.getenv('PORT','8765')); print(f'AI Snap Cloud v0.4.4 listening on :{port}'); ThreadingHTTPServer(('0.0.0.0',port),Handler).serve_forever()
