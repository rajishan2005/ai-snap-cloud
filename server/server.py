...
class Handler(BaseHTTPRequestHandler):
    server_version = 'AISnapCloud/0.3'
    def cors(self):
    ...
    def do_GET(self):
        ...
        if path == '/health': return self.send_json({'ok':True,'name':'AI Snap Cloud','version':'0.3.0'})
        if path == '/': return self.send_json({'ok':True,'name':'AI Snap Cloud','version':'0.3.0','mobile':f'{public_base(self)}/mobile/'})
        if path == '/api/pair/create':
            with lock:
                code = new_code(); pairs[code] = {'created':now(),'last_used':now()}
        ...
        if path.startswith('/api/file/'):
            ...
            if not f.exists() or not f.is_file(): return self.send_json({'error':'not found'},404)
    ...
    def do_POST(self):
        ...
        if path == '/api/upload':
            ...
            data=self.rfile.read(length); ext='.jpg' if 'jpeg' in ct else '.png' if 'png' in ct else '.webp' if 'webp' in ct else '.bin'
            name=f'snap_{int(now()*1000)}_{secrets.token_hex(3)}{ext}'; d=pair_dir(code); d.mkdir(exist_ok=True); (d/name).write_bytes(data); touch_pair(code)
