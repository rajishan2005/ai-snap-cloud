...
def notify(code, name):
    ...
    with lock:
        targets = list(subscribers.get(code, []))
    dead = []
    for wfile in targets:
        try:
            wfile.write(payload); wfile.flush()
        except Exception:
            dead.append(wfile)
    if dead:
        with lock:
            current = subscribers.get(code, [])
            subscribers[code] = [x for x in current if x not in dead]

def cleanup_loop():
    while True:
        time.sleep(CLEAN_INTERVAL)
        cutoff_file, cutoff_pair = now()-FILE_TTL, now()-PAIR_TTL
        with lock:
            expired = [c for c,v in pairs.items() if v['created'] < cutoff_pair]
            for c in expired:
                pairs.pop(c, None)
                subscribers.pop(c, None)
                d = pair_dir(c)
                if d.exists():
                    for p in d.iterdir():
                        try: p.unlink()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def cors(self):
        ...
        self.send_header('Access-Control-Allow-Headers','Content-Type, X-AI-Snap-Key, X-AI-Snap-IV, X-AI-Snap-Mime, X-AI-Snap-Name')
    ...
    def do_GET(self):
        ...
        if path == '/api/pair/create':
            with lock:
                code = new_code(); pairs[code] = {'created':now(),'last_used':now(),'public_key':None}
            pair_dir(code).mkdir(exist_ok=True); base = public_base(self)
            return self.send_json({'ok':True,'code':code,'expiresIn':PAIR_TTL,'mobileUrl':f'{base}/mobile/?code={code}'})
        if path == '/api/pair/qr':
            code = self.code()
        ...
        if path == '/api/events':
            ...
            try:
                ...
                with lock:
                    subscribers[code] = [x for x in subscribers.get(code, []) if x is not self.wfile]
            return
        ...
        if path.startswith('/api/file/'):
            ...
            return self.send_bytes(data,200,'application/octet-stream',{'X-AI-Snap-IV':info['iv'],'X-AI-Snap-Key':info['key'],'X-AI-Snap-Mime':info['mime'],'X-AI-Snap-Name':info['name']})
    ...
    def do_POST(self):
        ...
        if path == '/api/pair/create':
            try:
                length=int(self.headers.get('Content-Length','0')); body=json.loads(self.rfile.read(min(length,100_000)).decode())
            ...
            with lock:
                code = new_code(); pairs[code] = {'created':now(),'last_used':now(),'public_key':public_key}
            pair_dir(code).mkdir(exist_ok=True); base=public_base(self)
            return self.send_json({'ok':True,'code':code,'expiresIn':PAIR_TTL,'mobileUrl':f'{base}/mobile/?code={code}','e2ee':True})
        if path == '/api/upload':
            code=self.code()
            if not valid_code(code): return self.send_json({'error':'invalid_or_expired_pair'},401)
            ...
            name_header=self.headers.get('X-AI-Snap-Name','photo.jpg')
            safe_name=Path(urllib.parse.unquote(name_header)).name[:180] or 'photo.jpg'
            mime=self.headers.get('X-AI-Snap-Mime','image/jpeg')
            if not mime.startswith('image/'): return self.send_json({'error':'image_only'},415)
            name=f'snap_{int(now()*1000)}_{secrets.token_hex(3)}.enc'; d=pair_dir(code); d.mkdir(exist_ok=True)
            (d/name).write_bytes(data)
            name_b64=urllib.parse.quote(safe_name,safe='')
            ...
            touch_pair(code); notify(code,name)
            return self.send_json({'ok':True,'name':name,'encrypted':True})
...
def public_base(handler):
    ...
    scheme=forwarded.split(',')[0].strip() if forwarded else ('https' if os.getenv('RAILWAY_PUBLIC_DOMAIN') else 'http')
